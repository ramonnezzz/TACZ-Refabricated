...
public class GunAttachmentSlot extends Button implements IStackTooltip {
    ...
    @Override
    protected void extractContents(@NotNull GuiGraphicsExtractor graphics, int pMouseX, int pMouseY, float pPartialTick) {
        if (this.isHoveredOrFocused()) {
            ...
            graphics.centeredText(font, Component.translatable(nameKey), this.getX() + this.getWidth() / 2, yOffset, TextColor.WHITE.getValue());
        }
        ...
        if (isHoveredOrFocused() || selected) {
            graphics.blit(RenderPipelines.GUI_TEXTURED, GunRefitScreen.SLOT_TEXTURE, x, y, 0, 0, width, height, GunRefitScreen.SLOT_SIZE, GunRefitScreen.SLOT_SIZE);
        } else {
        ...
        if (!attachmentItem.isEmpty()) {
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, GunRefitScreen.ICONS_TEXTURE, x + 2, y + 2, xOffset, 0, width - 4, height - 4, GunRefitScreen.ICON_UV_SIZE, GunRefitScreen.ICON_UV_SIZE, GunRefitScreen.getSlotsTextureWidth(), GunRefitScreen.ICON_UV_SIZE);
        }
    }
